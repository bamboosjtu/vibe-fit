...
export function CardioSection() {
  const [cardioExercises, setCardioExercises] = useState<CardioData[]>([
    { id: 'c1', exerciseId: 'treadmill', name: '跑步机', lastRecord: '上次：30分钟 · 坡度 2.0', duration: '30', incline: '2.0', speed: '8.5', calories: '320', isActive: false, isPaused: false },
    { id: 'c2', exerciseId: 'elliptical', name: '椭圆机', lastRecord: '', duration: '25', incline: '8', speed: '', calories: '', isActive: false, isPaused: false },
    { id: 'c3', exerciseId: 'rowing-machine', name: '划船机', lastRecord: '上次：2000米 · 8:45', duration: '', incline: '', speed: '', calories: '', isActive: false, isPaused: false },
  ]);
      ...
      {cardioExercises.map((cardio) => (
        <Card 
          ...
          sx={{ 
            mb: 2,
            borderRadius: '12px',
            border: cardio.isActive ? '2px solid' : '1px solid',
            borderColor: cardio.isActive ? 'primary.main' : 'divider',
            boxShadow: cardio.isActive ? '0 14px 34px rgba(16, 185, 129, 0.14)' : '0 10px 28px rgba(15, 23, 42, 0.06)',
          }}
        >
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                <Box sx={{ color: 'text.primary' }}>
                  <ExerciseArtwork exerciseId={cardio.exerciseId} exerciseName={cardio.name} type="cardio" size={64} />
                </Box>
                <Box>
                  <Typography variant="h6" fontWeight="bold" sx={{ fontFamily: 'var(--font-display)', fontSize: '1rem' }}>{cardio.name}</Typography>
