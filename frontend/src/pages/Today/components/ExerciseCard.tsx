...
export function ExerciseCard({ sessionExercise }: ExerciseCardProps) {
  ...
  return (
    ...
    >
      <CardContent sx={{ p: 2, pb: isExpanded ? 1.5 : 1 }}>
        {/* Card Header */}
        <Box 
        ...
        >
          ...
          <Box sx={{ flex: 1 }}>
            <Typography variant="h6" fontWeight="bold" sx={{ fontSize: '1rem', fontFamily: '"Poppins", sans-serif' }}>
...
function TableHeader() {
  return (
    <Box sx={{ 
      display: 'grid', gridTemplateColumns: '36px 1fr 1fr 36px 36px', gap: 0.5, mb: 0.75, px: 0.5, py: 0.5,
      bgcolor: 'rgba(16, 185, 129, 0.05)', borderRadius: '6px'
    }}>
      {['组', '重量', '次数', '✓', ''].map((label, i) => (
...
function SetRow({ set, onToggle, onUpdate, onDelete }: { 
  ...
  return (
    <Box sx={{ 
      display: 'grid', gridTemplateColumns: '36px 1fr 1fr 36px 36px', gap: 0.5, px: 0.5, py: 0.75, mb: 0.5,
...
const actionButtonStyle = {
  borderRadius: '8px', py: 0.75, borderWidth: 1.5, borderColor: 'primary.light', color: 'primary.main',
